      ...
      <div className="exam-accordion-list">
        ...
        >
          <form className="ai-generator-form" onSubmit={generateWithAi}>
            ...
            <div className="ai-source-note"><b>مصادر التوليد</b><span>المقرر: {exam.subjectName || exam.subjectCode || "المقرر المرتبط بالاختبار"}</span><span>وصف الاختبار: {exam.description || exam.topic || "غير محدد"}</span><span>المخطط: {settings.assessmentType === "CloAligned" ? "CLO + Bloom" : "عام + Bloom"}</span><span>PDF: {aiPdf?.name || "بدون ملف"}</span><span>{aiPdf ? "يستخرج النظام النص ويكتشف لغته، ثم يلخصه وينشئ الأسئلة باللغة نفسها دون ترجمة. لا يُحفظ ملف PDF على الخادم." : "سيتم إنشاء الأسئلة من وصف الاختبار ومخطط الورقة."}</span></div>
